...
A conversation agent (either `Llama3-8b-instruct` or `MentaLLaMA-chat-7B` depending on notebook) will carry out a conversation with patients to gather relevant (simulated via `Llama3-8b-instruct` trained on real patients with differing depression scores), after which an evaluator agent (also based on `Llama3-8b-instruct`) will predict a BDI-II depression score and a set of relevant symptoms.
